    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.empleados.update", [$empleado->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('last_names') ? 'is-invalid' : '' }}" type="text" name="last_names" id="last_names" value="{{ old('last_names', $empleado->last_names) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('correo') ? 'is-invalid' : '' }}" type="email" name="correo" id="correo" value="{{ old('correo', $empleado->correo) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('unidad_de_negocio') ? 'is-invalid' : '' }}" name="unidad_de_negocio_id" id="unidad_de_negocio_id" required>
                    @foreach($unidad_de_negocios as $id => $entry)
                        <option value="{{ $id }}" {{ (old('unidad_de_negocio_id') ? old('unidad_de_negocio_id') : $empleado->unidad_de_negocio->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
