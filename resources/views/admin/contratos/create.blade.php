...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.contrato.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contratos.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('contratodesde') ? 'is-invalid' : '' }}" type="text" name="contratodesde" id="contratodesde" value="{{ old('contratodesde') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('contratohasta') ? 'is-invalid' : '' }}" type="text" name="contratohasta" id="contratohasta" value="{{ old('contratohasta') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('contratoestado') ? 'is-invalid' : '' }}" name="contratoestado" id="contratoestado" required>
                    ...
                    @foreach(App\Models\Contrato::CONTRATOESTADO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('contratoestado', 'enabled') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
